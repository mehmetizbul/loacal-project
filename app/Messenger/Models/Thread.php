...
class Thread extends Eloquent
{
     ...
     * @var array
     */
    protected $fillable = ['booking_request'];
    ...
    {
        return $this->hasMany("App\Messenger\Models\Message", 'thread_id', 'id');
    }
    ...
    {
        return $this->hasOne("App\BookingRequest", 'id', 'booking_request')->first();
    }
    ...
    {
        return $this->hasMany("App\Messenger\Models\Participant", 'thread_id', 'id');
    }
    ...
    /**
     * Returns all threads by booking_request.
     *
     * @return mixed
     */
    public static function getByBookingRequest($bookingRequestQuery)
    {
        return self::where('booking_request', 'like', $bookingRequestQuery)->get();
    }
    ...
    public function scopeForUser($query, $userId)
    {
        ...
        $bookingrequestTable = "booking_requests";

        return $query
            ->join($participantsTable, $this->getQualifiedKeyName(), '=', $participantsTable . '.thread_id')
            ->join($bookingrequestTable, 'booking_request', '=', $bookingrequestTable . '.id')
            ->where($participantsTable . '.user_id', $userId)
            ->where($participantsTable . '.deleted_at', null)
            ->where($bookingrequestTable . '.closed',0)
            ->select($threadsTable . '.*');
    }
    ...
    public function participantsString($userId = null, $columns = ['name'])
    {
        $participantsTable = 'participants';
        $usersTable = 'users';
        $userPrimaryKey = static::make(["App\User"])->getKeyName();
